...
// lower sqlite log level when testing
pub fn log(comptime level: std.log.Level, comptime scope: @TypeOf(.EnumLiteral), comptime format: []const u8, args: anytype) void {
	var buffer: [64]u8 = undefined;
	const stderr = std.debug.lockStderr(&buffer).terminal();
	defer std.debug.unlockStderr();

	const lvl: std.log.Level = if (builtin.is_test and scope == .sqlite and level == .err) .warn
		else level;

	return std.log.defaultLogFileTerminal(lvl, scope, format, args, stderr) catch {};
}
...
const Env = struct {
	...
	fn readEnvBool(map: std.process.Environ.Map, key: []const u8, deflt: bool) bool {
		const value = map.get(key) orelse return deflt;
